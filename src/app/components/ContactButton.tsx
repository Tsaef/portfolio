...
export default function ContactButton() {
  ...
  return (
    <>
      ...
      <div
        className={`fixed bottom-24 right-0 z-40 bg-gray-800 rounded-lg shadow-2xl transition-transform duration-300 ease-in-out ${
          isContactPanelOpen ? "translate-x-0 right-6" : "translate-x-full"
        }`}
        style={{ width: "280px" }}
      >
        <div className="p-4">
          <div className="flex items-center justify-between mb-4">
            ...
            <button
              ...
              className="p-1 hover:bg-gray-700 hover:cursor-pointer rounded-full transition-colors"
            >
          ...
          <div className="space-y-3">
            {contact.map((contactItem, index) => (
              <a
                ...
                target={contactItem.type === 'email' ? '_self' : '_blank'}
                rel={contactItem.type === 'email' ? undefined : 'noopener noreferrer'}
              ...
              >
                ...
                <div>
                  <p className="text-sm font-medium text-white">{contactItem.label}</p>
                  <p className="text-xs text-gray-300">{contactItem.subtitle}</p>
                </div>
              </a>
            ))}
          </div>
      ...
      <div className="fixed flex bottom-6 right-6 z-50">
        ...
        >
          <EnvelopeIcon className="w-6 h-6 text-white flex-shrink-0" />
          <span className="hidden lg:flex text-white whitespace-nowrap">
            Contact
          </span>
        </div>
      </div>
